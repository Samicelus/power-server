...
class BaseHandler{
    ...
    async restError(ctx, error_code, error_message){
        if(ctx.log_param){
            let request_address = ctx.ip.split(':');
            let ip = request_address[request_address.length-1];
            let user = ctx.request.user_info;
    ...
    async download(ctx, {file, option}){
        ctx.response.attachment(file, option);
    }
}
